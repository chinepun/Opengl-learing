...
void Model::loadModel(const std::string &path)
{
    ...
    const aiScene *scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs |
                          aiProcess_JoinIdenticalVertices | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace);
    // check for errors
...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        if (mesh->mTextureCoords[0])// does the mesh contain texture Co-ordinates
        {
            glm::vec2 vec;
...
unsigned int TextureFromFile(const char *path, const std::string &directory, bool gamma)
{
    ...
    filename = directory + '/' + filename;std::cout << filename << '\t';
//filename = "../model/backpack.obj";
    ...
    {
        std::cout << "Texture m failed to load at path: " << path << std::endl;
        stbi_image_free(data);
    }
